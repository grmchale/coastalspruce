...
#Joined spectral library & canopy VIs
df_rf<-speclib_rf %>% 
...
predictor_vars <- names(canopy_VIs)[12:108]
...
trainIndex <- createDataPartition(model_data[[response_var]], p = 0.8, list = FALSE)
...
rf_model <- ranger(
  ...
  data = train_data,
  num.trees = 500,    # 500 is reasonable?
