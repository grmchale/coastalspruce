...
#Set directory
path <- ("./R_outputs/canopy_spectra_dendrometers/")
...
Canopy_labeled<-lapply(1:length(imgs), function(x){ 
  ...
  df<-spectrolab::resample(df, new_bands = seq(398, 999, 1),fwhm=1) #parallel = FALSE)
  
  #parse metadata from file name
  imgs_names<-gsub(".ENVI","", imgs_names) %>% as.data.frame()
  TrID<-separate(data.frame(A = imgs_names), col = "." , into = c("Site","Species","X", "Y", "CC"), sep = "_")
